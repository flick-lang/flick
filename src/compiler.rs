...
impl<'a> Compiler<'a> {
    ...
    unsafe fn codegen_statement(&mut self, statement: &Statement) -> LLVMValueRef {
        match statement {
    ...
    unsafe fn codegen_var_declaration(&mut self, var_declaration: &VarDeclaration) -> LLVMValueRef {
    ...
    unsafe fn codegen_while_loop(&self, while_loop: &WhileLoop) -> LLVMValueRef {
        todo!()
    }

    unsafe fn codegen_expr(&self, expr: &Expr) -> LLVMValueRef {
        match expr {
    ...
    unsafe fn codegen_identifier(&self, identifier: &str) -> LLVMValueRef {
    ...
    unsafe fn codegen_int(&self, value: i64) -> LLVMValueRef {
    ...
    unsafe fn codegen_bin_expr(&self, bin_expr: &BinExpr) -> LLVMValueRef {
    ...
    unsafe fn codegen_func_def(&mut self, func_def: &FuncDef) -> LLVMValueRef {
